    ...
    public class SystemAuthen : ActionFilterAttribute
    {
        ...
        private void LayThongTinCaiDat(ActionExecutingContext context)
        {
            ...
            if (EscsUtils.cai_dat == null || EscsUtils.cai_dat.Count()<=0 || EscsUtils.cai_dat.Where(n=>n.domain == domainName).Count()<=0)
            {
                ...
                var baseResponse = context.HttpContext.Request.GetRespone<ht_cai_dat>(StoredProcedure.PBH_CAI_DAT_UNG_DUNG_LKE, new { domain = domainName }).Result;
        ...
        private void LayThongTinMenu(ActionExecutingContext context, escs_nguoi_dung user)
        {
            ...
            {
                var baseResponse = context.HttpContext.Request.GetRespone<escs_authen>(StoredProcedure.PHT_NSD_LOGIN, new { ma_doi_tac_nsd = user.ma_doi_tac, nsd = user.nsd, pas = user.pas }).Result;
