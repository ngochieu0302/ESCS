...
/// <reference path="services/motoinvestigationservice.js" />
...
const MAN_HINH  = 'GIAM_DINH'; //Giám định
...
var arrNV_CT = [
    { ma: "KIEN_NGHI", ten: "Kiến nghị giải quyết", nv: "XE_MAY", pm: "GD" },
    { ma: "TRINH_DUYET_GD", ten: "Trình báo cáo giám định", nv: "XE_MAY", pm: "GD" }
]
...
var _motoCompensationService = new MotoCompensationService();
...
var _modalMotoSearch = new ModalService("modalMotoSearch");
var _modalMotoCustomerInfo = new ModalService("modalMotoCustomerInfo");
...
var _modalXemHinhAnh = new ModalFullScreenService("modalXemHinhAnh", "navDanhGiaTonThat");
...
var _modalMotoClaimCompareData = new ModalFullScreenService("modalMotoClaimCompareData", "stepHinhAnhHoSo");
...
var _modalLapPhuongAnSuaChuaXeMayService = new ModalLapPhuongAnSuaChuaService();
...
var _frmMotoClaimMotoSearch = new FormService("frmMotoClaimMotoSearch");
var _frmMotoClaimCustomerInfo = new FormService("frmMotoClaimCustomerInfo");
...
var DS_NGHIEP_VU = _commonService.danhMucChung.nghiep_vu.where(n => n.ap_dung == 1);
...
function timKiemXe() {
    var objSearch = _frmMotoClaimMotoSearch.getJsonData();
    objSearch.nv = "XE_MAY";
    ...
    _service.timKiemXe(objSearch).then(res => {
        ...
        ESUtil.genHTML("modalMotoSearchDsGCNTemplate", "modalMotoSearchDsGCN", { data_info: data });
    });
}
...
function selectMoto(obj = undefined) {
    _frmMotoClaimCustomerInfo.clearErrorMessage();
}
...
function setNguoiLienHeTheoNguoiThongBao() {
    _frmMotoClaimCustomerInfo.getControl("nguoi_lhe").readOnly(false);
    _frmMotoClaimCustomerInfo.getControl("moi_qh_lhe").readOnly(false);
    _frmMotoClaimCustomerInfo.getControl("dthoai_lhe").readOnly(false);
    _frmMotoClaimCustomerInfo.getControl("email_lhe").readOnly(false);
    if (_frmMotoClaimCustomerInfo.getControlById("chkThamGiaLienHe").is(":checked")) {
        var customer = _frmMotoClaimCustomerInfo.getJsonData();
        ...
        _frmMotoClaimCustomerInfo.setData(customer);
        _frmMotoClaimCustomerInfo.getControl("nguoi_lhe").readOnly();
        _frmMotoClaimCustomerInfo.getControl("moi_qh_lhe").readOnly();
        _frmMotoClaimCustomerInfo.getControl("dthoai_lhe").readOnly();
        _frmMotoClaimCustomerInfo.getControl("email_lhe").readOnly();
    }
}
...
function kiemTraChuyenStep(step) {
    ...
    if (step === "stepDanhGiaTonThat") {
        var lhnv = objDanhMuc.lhnv.where(n => n.ma == ho_so_chi_tiet.data_info.ho_so.nv_xly).firstOrDefault();
        if (lhnv != null && lhnv.doi_tuong != 'NGUOI' && lhnv.doi_tuong != 'NGUOI_HK') {
...
function bindCmbDataDonVi(objDanhMuc) {
    ...
    _frmMotoClaimMotoSearch.getControl("ma_doi_tac").setDataSource(objDanhMuc.doi_tac, "ten", "ma", "Chọn đối tác", ESCS_MA_DOI_TAC);
    _frmMotoClaimMotoSearch.getControl("ma_chi_nhanh").setDataSource(arrChiNhanh, "ten", "ma", "Chọn chi nhánh", "");
    _frmMotoClaimMotoSearch.getControl("ma_doi_tac").addEventChange(val => {
        var arrChiNhanh = objDanhMuc.chi_nhanh.where(n => n.ma_doi_tac === val);
        _frmMotoClaimMotoSearch.getControl("ma_chi_nhanh").setDataSource(arrChiNhanh, "ten_tat", "ma", "Chọn chi nhánh", "");
        _frmMotoClaimMotoSearch.getControl("ma_chi_nhanh").setValue("");
    });
...
function bindCmbDataMoiQuanHe(objDanhMuc) {
    ...
    _frmMotoClaimCustomerInfo.getControl("moi_qh_tb").setDataSource(arrMoiQuanHe, "ten", "ma", "Chọn mối quan hệ", "");
    _frmMotoClaimCustomerInfo.getControl("moi_qh_lhe").setDataSource(arrMoiQuanHe, "ten", "ma", "Chọn mối quan hệ", "");
...
function anHienPhanLoai(hien = true) {
    if (hien) {
        $("#divMotoCommonInfo").hide();
        $("#divMotoClaimContentStep3HinhAnh").hide();
        $("#divMotoClaimContentStep3PhanLoai").show();
        anHienXemAnhPhanLoaiNhanh(false);
    }
    else {
        $("#divMotoCommonInfo").show();
        $("#divMotoClaimContentStep3HinhAnh").show();
        $("#divMotoClaimContentStep3PhanLoai").hide();
        anHienXemAnhPhanLoaiNhanh(true);
    }
}
function onChonKhachHang(so_id, so_id_dt, loai) {
    $("#modalMotoSearchDsGCN .tkiem_kh").removeClass("active");
    $("#modalMotoSearchDsGCN #tkiem_kh_" + so_id + "_" + so_id_dt + "_" + loai).addClass("active");
}
...
function tinhTongTienNguoi() {
    ...
    if (nhom == "NG" && doi_tuong == "NGUOI") {
        $("#modalChiTietTonThatNGUOITongTienTT").html(ESUtil.formatMoney(tong_tien_tt));
    }
...
function suaDoiTuongTT(so_id_doi_tuong) {
    ...
    if (dt.nhom == DOI_TUONG_TT.TAI_SAN) {
        _frmAddDoiTuongTT.getControl("loai").setDataSource([{ ma: "KHAC", ten: "Tài sản khác" }, { ma: "XE", ten: "Tài sản là một xe mô tô khác" }], "ten", "ma", "Chọn loại đối tượng", "");
    }
...
function xemThongTinGiayChungNhan(ma_chi_nhanh, so_id_hdong, so_id_gcn) {
    ...
    _service.base.all([
        _motoClaimCommonService.layThongTinGCN(obj),
...
function layThongTinOCRBaoGia(obj, callback = undefined) {
    arrDataBaoGia = [];
    _motoCompensationService.layDuLieuBaoGiaGara(obj).then(res => {
...
function navTimKiemXe(val) {
    ...
    _frmTaoHDAo.getControl("nv").setValue("XE_MAY");
    _frmTaoHDAo.getControl("nv").readOnly();
    _frmTaoHDAo.getControl("loai").setValue("TN");
    _frmTaoHDAo.getControl("loai").trigger("select2:select");
    if (val == "TAOHD") {
        _frmMotoClaimCustomerInfo.getControl("so_id_hd").setValue("");
        _frmMotoClaimCustomerInfo.getControl("so_id_dt").setValue("");
    }
}
//Chọn gara sửa chữa
...
$(document).ready(function () {
    ...
    if (storageDonViHanhChinh === undefined || storageDonViHanhChinh === null || storageDonViHanhChinh === "null") {
        _service.base.all([
            ...
            _commonService.layControl({ nv: "XE_MAY" }),//2
            _service.timKiemHoSo(objTimKiem),//3
            _partnerListService.layDsDoiTac(),//4
            _branchListService.layDsChiNhanh(),//5
            _businessCodeService.layDsLHNV("XE_MAY"),//6
            _categoryvehicleListService.layDsHangMucXe({ nv: "XE_MAY" }),//7
            ...
            _statusListService.layDsTrangThai({ nv: "XE_MAY" }),//12
            ...
            _service.dsNhomSuKienBH({ nv: "XE_MAY" }),//17
            ...
            _carManufacturerListService.layDsHangXe({ nv: "XE_MAY" }),//21
            _hieuXeService.layDsHieuXe({ nv: "XE_MAY" }),//22
            _rangeVehicleService.layDsLoaiXe({ nv: "XE_MAY" }),//23,
            _storageUnitService.layDsMaKho({ ma_doi_tac: ESCS_MA_DOI_TAC }), //24
            _motoClaimCommonService.lietKeDanhSachGiayToOCR({ ma_doi_tac: ESCS_MA_DOI_TAC }),//25
        ]).then(arrRes => {
            ...
            objDanhMuc.hang_xe = arrRes[21].data_info.where(n => n.nv == 'XE_MAY');
            objDanhMuc.hieu_xe = arrRes[22].data_info.where(n => n.nv == 'XE_MAY');
            objDanhMuc.loai_xe = arrRes[23].data_info.where(n => n.nv == 'XE_MAY');
        ...
        _service.base.all([
            ...
            _commonService.layControl({ nv: "XE_MAY" }),
            _service.timKiemHoSo(objTimKiem),
            _partnerListService.layDsDoiTac(),
            _branchListService.layDsChiNhanh(),
            _businessCodeService.layDsLHNV("XE_MAY"),
            _categoryvehicleListService.layDsHangMucXe({nv: "XE_MAY"}),
            ...
            _statusListService.layDsTrangThai({ nv: "XE_MAY" }),
            ...
            _carManufacturerListService.layDsHangXe({ nv: "XE_MAY" }),
            _hieuXeService.layDsHieuXe({ nv: "XE_MAY" }),
            _rangeVehicleService.layDsLoaiXe({ nv: "XE_MAY" }),
            _storageUnitService.layDsMaKho({ ma_doi_tac: ESCS_MA_DOI_TAC }),
            _motoClaimCommonService.lietKeDanhSachGiayToOCR({ ma_doi_tac: ESCS_MA_DOI_TAC })
        ]).then(arrRes => {
            objDanhMuc.can_bo = arrRes[0].data_info;
            ...
            objDanhMuc.hang_xe = arrRes[20].data_info.where(n => n.nv == 'XE_MAY');
            objDanhMuc.hieu_xe = arrRes[21].data_info.where(n => n.nv == 'XE_MAY');
            objDanhMuc.loai_xe = arrRes[22].data_info.where(n => n.nv == 'XE_MAY');
    ...
    _frmMotoClaimCustomerInfo.getControlById("chkThamGiaLienHe").change(function () {
        setNguoiLienHeTheoNguoiThongBao();
    });
    _frmMotoClaimCustomerInfo.getMultipleControl("nguoi_tb, dthoai_tb, email_tb").keyup(function () {
        setNguoiLienHeTheoNguoiThongBao();
    });
    _frmMotoClaimCustomerInfo.getControl("moi_qh_tb").addEventChange(val => {
        if (val == 'QH.0001') {
            var obj = {
                ma_doi_tac: _frmMotoClaimCustomerInfo.getControl('ma_doi_tac').val(),
                ma_chi_nhanh: '',
                so_id: _frmMotoClaimCustomerInfo.getControl('so_id_hd').val()
            }
            _carContractService.layChiTietHopDong(obj).then(res => {
                ...
                _frmMotoClaimCustomerInfo.getControl('nguoi_tb').setValue(res_data.nguoi_tb);
                _frmMotoClaimCustomerInfo.getControl('dthoai_tb').setValue(res_data.dthoai_tb);
                _frmMotoClaimCustomerInfo.getControl('email_tb').setValue(res_data.email_tb);
            });
        }
        setNguoiLienHeTheoNguoiThongBao();
    });
    ...
    _frmAddDoiTuongTT.getControl("nhom").addEventChange(val => {
        ...
        if (val == DOI_TUONG_TT.TAI_SAN) {
            _frmAddDoiTuongTT.getControl("loai").setDataSource([
                ...
                { ma: "XE", ten: "Tài sản là một xe mô tô khác" }
            ], "ten", "ma", "Chọn loại đối tượng", "");
    ...
    $("#btnThemMoiHoSo").click(function () {
        ...
        _frmMotoClaimMotoSearch.resetForm();
        _frmMotoClaimMotoSearch.clearErrorMessage();
        _frmMotoClaimCustomerInfo.resetForm();
        _frmMotoClaimCustomerInfo.clearErrorMessage();
        _frmMotoClaimMotoSearch.getControl("ma_doi_tac").setValue(ESCS_MA_DOI_TAC);
        _frmMotoClaimMotoSearch.getControl("ma_doi_tac").trigger("select2:select");
        _frmMotoClaimMotoSearch.getControl("ngay_xr").setValue(new Date().ddmmyyyy());

        ESUtil.genHTML("modalMotoSearchDsGCNTemplate", "modalMotoSearchDsGCN", { data_info: [] });
        $("#modalMotoSearch").modal("show");
        _navTabTimKiemXe.showTab("tabTimKiemXe");
        showStep('tabTimKiemXe');
    });
    $("#btnMotoSearch").click(function () {
        if (!_frmMotoClaimMotoSearch.isValid())
            return;
        timKiemXe();
    });
    $("#btnTiepTheo").click(function () {
        ...
        var currentTab = _navTabTimKiemXe.currentTab;
        var selectedData = [];
        if ($("#modalMotoSearchDsGCN .tkiem_kh.active").length > 0) {
            selectedData.push({
                ma_doi_tac: $("#modalMotoSearchDsGCN .tkiem_kh.active").attr("data_ma_doi_tac"),
                ma_chi_nhanh: $("#modalMotoSearchDsGCN .tkiem_kh.active").attr("data_ma_chi_nhanh"),
                so_id: $("#modalMotoSearchDsGCN .tkiem_kh.active").attr("data_so_id"),
                so_id_dt: $("#modalMotoSearchDsGCN .tkiem_kh.active").attr("data_so_id_dt"),
                loai: $("#modalMotoSearchDsGCN .tkiem_kh.active").attr("data_loai")
            });
        }
        if (currentTab === "tabTimKiemXe") {
            ...
            if (tabActive == "TIMKIEM") {
                if (selectedData.length === 1) {
                    if (_frmMotoClaimMotoSearch.isValid()) {
                        ...
                        if (_frmMotoClaimCustomerInfo.getControl('so_id').val() == '' || _frmMotoClaimCustomerInfo.getControl('so_id').val() == undefined || _frmMotoClaimCustomerInfo.getControl('so_id').val() == null) {
                            _frmMotoClaimCustomerInfo.getControl('nguon_tb').attr('readonly', false);
                            _frmMotoClaimCustomerInfo.getControl('hien_truong').attr('readonly', false);
                            _frmMotoClaimCustomerInfo.getControl('gio_tb').prop('readonly', false);
                            _frmMotoClaimCustomerInfo.getControl('ngay_tb').prop('readonly', false);

                            _frmMotoClaimCustomerInfo.getControl("nguon_tb").setValue("TTGD");
                            _frmMotoClaimCustomerInfo.getControl("hien_truong").setValue("K");
                            _frmMotoClaimCustomerInfo.getControl("gio_tb").val(gioHT);
                            _frmMotoClaimCustomerInfo.getControl("ngay_tb").val(dateNow);
                        }

                        _frmMotoClaimCustomerInfo.getControl("nv").val(dataRow.loai);
                        _frmMotoClaimCustomerInfo.getControl("ma_doi_tac").val(ESCS_MA_DOI_TAC);
                        _frmMotoClaimCustomerInfo.getControl("ma_chi_nhanh").val(dataRow.ma_chi_nhanh);
                        _frmMotoClaimCustomerInfo.getControl("so_id_hd").val(dataRow.so_id);
                        _frmMotoClaimCustomerInfo.getControl("so_id_dt").val(dataRow.so_id_dt);
                        _frmMotoClaimCustomerInfo.getControl("nv_xly").setDataSource(_commonService.danhMucChung.luong_xly, "ten", "ma", "Chọn luồng xử lý", "");
                        ...
                        _motoClaimCommonService.layThongTinGCN(objRequest).then(res => {
                            if (res.state_info.status !== "OK") {
                                _notifyService.error(res.state_info.message_body);
                                return;
                            }
            ...
            if (tabActive == "TAOHD") {
                ...
                _frmMotoClaimCustomerInfo.getControl("nv_xly").setDataSource(lhnv, "ten", "ma", "Chọn luồng xử lý", obj.lh_nv);
                _navTabTimKiemXe.showTab("tabThongTinLienHe");
            }
            _frmMotoClaimCustomerInfo.getControl("nguon_tb").setValue("TTGD");
            _frmMotoClaimCustomerInfo.getControl("hien_truong").setValue("K");
            ...
            _frmMotoClaimCustomerInfo.clearErrorMessage();
            _frmMotoClaimCustomerInfo.getControl('nguon_tb').removeAttr('readonly');
            _frmMotoClaimCustomerInfo.getControl('gio_tb').prop('readonly', false);
            _frmMotoClaimCustomerInfo.getControl('ngay_tb').prop('readonly', false);
            if (_frmMotoClaimCustomerInfo.isValid()) {
                var objJson = _frmMotoClaimCustomerInfo.getJsonData();
                ...
                _service.luuThongTinNguoiThongBao(objJson).then(res => {
                    ...
                    _frmMotoClaimCustomerInfo.getControl("so_hs").val(res.out_value.so_hs);
                    _frmMotoClaimCustomerInfo.getControl("so_id").val(res.out_value.so_id);
    ...
    $("#btnSuaThongTinXeTonThat").click(function () {
        ...
        obj.ngay_tb = ngay_tb_tmp[0].dateToNumber();
        obj.gio_tb = ngay_tb_tmp[1];
        _frmMotoClaimCustomerInfo.setData(obj);
        _navTabTimKiemXe.showTab("tabTimKiemXe");
        navTimKiemXe('TIMKIEM');
        _frmMotoClaimMotoSearch.getControl("ma_doi_tac").setValue(ESCS_MA_DOI_TAC);
        _frmMotoClaimMotoSearch.getControl("ma_doi_tac").trigger("select2:select");

        _frmMotoClaimCustomerInfo.getControl('nguon_tb').attr('readonly', 'readonly');
        _frmMotoClaimCustomerInfo.getControl('hien_truong').attr('readonly', 'readonly');
        _frmMotoClaimCustomerInfo.getControl('gio_tb').prop('readonly', true);
        _frmMotoClaimCustomerInfo.getControl('ngay_tb').prop('readonly', true);
        _modalMotoSearch.show();
    });
    ...
    $("#btnXoaLoadAnhDGTT").click(function () {
        ...
        _notifyService.confirmDelete("Bạn có chắc chắn muốn xóa các ảnh đã chọn không?", index, val => {
            _service.xoaAnhHoSoGiamDinh({ so_id: ho_so_chi_tiet.data_info.ho_so.so_id, bt: arrVal, pm: CONSTANT_PM, nv: "XE_MAY"}).then(res => {
                if (res !== undefined && res !== null && res.state_info.status === "NotOK") {
                    _notifyService.error(res.state_info.message_body);
                    return;
                }
    ...
    $("#btnInDGHT").click(function () {
        var sourceMauIn = [
            { ma: "ESCS_TBTN_YCBT_XE_MOTO", ten: "Thông báo tai nạn và yêu cầu bồi thường" },
            { ma: "ESCS_BCGD_XE_MOTO", ten: "Báo cáo giám định" },
            { ma: "ESCS_BBGD_HIEN_TRUONG_XE_MOTO", ten: "Biên bản ghi nhận hiện trường" },
            { ma: "ESCS_BBGD_XAC_DINH_THIET_HAI_XE_MOTO", ten: "Biên bản giám định/xác định thiệt hại xe mô tô" }
        ];
        var ma_mau_in_hthi = "ESCS_BBGD_HIEN_TRUONG_XE_MOTO";
    ...
    $("#btnInBCGD").click(function () {
        ...
        var ma_mau_in_hthi = "ESCS_BCGD_XE_MOTO";
        _modalDocumentService.setDataSource(sourceMauIn);
        _modalDocumentService.onClickIem = function (ma_mau_in) {
            _commonService.InPdf({
                ma_mau_in: ma_mau_in,
    ...
    $("#btnIn").bind("click", function () {
        ...
        var ma_mau_in_hthi = "ESCS_BBGD_XAC_DINH_THIET_HAI_XE_MOTO";
        _modalDocumentService.setDataSource(sourceMauIn);
        _modalDocumentService.onClickIem = function (ma_mau_in) {
            _commonService.InPdf({
                ma_mau_in: ma_mau_in,
    ...
    $("#btnHuyDuyetBCGD, #btnModalHuyDuyetBCGD").click(function () {
        _notifyService.confirm("Bạn có muốn hủy duyệt báo cáo giám định?", index, val => {
            ...
            obj.so_id = ho_so_chi_tiet.data_info.ho_so.so_id;
            obj.pm = CONSTANT_PM;
            obj.remove_file = "ESCS_BCGD_XE_MOTO";
    ...
    $("#btnDuyetBCGD, #btnModalDuyetBCGD").click(function () {
        _notifyService.confirm("Bạn có muốn duyệt báo cáo giám định?", "", type => {
            ...
            obj.create_file_sign = "ESCS_BCGD_XE_MOTO";
            obj.remove_file = "ESCS_BCGD_XE_MOTO";
    ...
    $("#btnTrinhphancapBCGD, #btnModalTrinhphancapBCGD").click(function () {
        var obj = {
            ...
            ma_doi_tac_ql: ho_so_chi_tiet.data_info.ho_so.ma_doi_tac_ql,
            so_id: ho_so_chi_tiet.data_info.ho_so.so_id,
            loai_trinh: "XE_MAY_TRINH_DUYET_BAO_CAO_GD",
            nghiep_vu: "XE_MAY",
            create_file: "ESCS_BCGD_XE_MOTO",
            remove_file: "ESCS_BCGD_XE_MOTO",
            pm: CONSTANT_PM
        }
    ...
    $("#btnTrinhphancap").click(function () {
        var obj = {
            ...
            ma_doi_tac_ql: ho_so_chi_tiet.data_info.ho_so.ma_doi_tac_ql,
            so_id: ho_so_chi_tiet.data_info.ho_so.so_id,
            loai_trinh: "XE_MAY_TRINH_DUYET_GIAM_DINH",
            nghiep_vu: "XE_MAY",
    ...
    $("#btnTrinhTuChoiBT").click(function () {
        var obj = {
            ...
            ma_doi_tac_ql: ho_so_chi_tiet.data_info.ho_so.ma_doi_tac_ql,
            so_id: ho_so_chi_tiet.data_info.ho_so.so_id,
            loai_trinh: "XE_MAY_TRINH_DUYET_TU_CHOI",
            nghiep_vu: "XE_MAY",
    ...
    $('#btnThemDoiTuongGiamDinh').click(function () {
        $("#btnMoiAddDoiTuongTT").removeClass("d-none");
        $("#btnLuuChonDoiTuongTT").removeClass("d-none");
        $("#btnLuuAddDoiTuongTT").addClass("d-none");
        $("#btnHuyLuu").addClass("d-none");
    ...
    $("#btnOCR").click(function () {
        ...
        _motoClaimCommonService.docOCR({ ma_doi_tac: ho_so_chi_tiet.data_info.ho_so.ma_doi_tac, so_id: ho_so_chi_tiet.data_info.ho_so.so_id, bt: arr_bt }).then(res => {
            if (res.state_info.status !== "OK") {
                _notifyService.error(res.state_info.message_body);
                return;
            }
    ...
    $("#btnAI").click(function () {
        ...
        _motoClaimCommonService.nhanDienAI({ so_id: ho_so_chi_tiet.data_info.ho_so.so_id, bt: arr_bt }).then(res => {
            if (res.state_info.status !== "OK") {
                _notifyService.error(res.state_info.message_body);
                return;
            }
    ...
    $("#btnThemSuKienBaoHiem").click(function () {
        ...
        _frmTKiemSuKienBH.getControl("nv").setValue("XE_MAY");
    ...
    $('#btnCapNhatUocTonThat').click(function () {
        ...
        _motoClaimCommon.showUocTonThat(obj);
    });
    ...
    $("#btnGuiEmailDGHT").click(function () {
        ...
        _notifyService.confirm("Bạn có chắc chắn muốn gửi thông báo xác nhận đến cho khách hàng hay không?", "", val => {
            var obj = {
                ...
                so_id: ho_so_chi_tiet.data_info.ho_so.so_id,
                nv: "XE_MAY",
                loai: "TEMPLATE_EMAIL_XAC_NHAN_DGHT_XE_MAY",
    ...
    $('#btnChonHangGPLX').click(function () {
        ...
        var val = arrChecked.join(",");
        if (val.includes("A4")) {
            _frmAddLicenseInfo.getControl("gplx_het_han").attr("required", true);
            $("#frmAddLicenseInfo").find("label.gplx_het_han").addClass("_required");
        } else {
            _frmAddLicenseInfo.getControl("gplx_het_han").attr("required", false);
            $("#frmAddLicenseInfo").find("label.gplx_het_han").removeClass("_required");
        }
        $(target).val(val);
        _modalHangGPLX.hide();
    });
    ...
    $("#btnDsGaraHopTac").inlinePopover({
        ...
        onBeforeShow: function (source) {
            ...
            _motoCompensationService.layGaraHopTac(obj).then(res => {
    ...
    $("#btnLuuChonGara").click(function () {
        ...
        _motoCompensationService.chonGaraHopTacBaoGia(obj).then(res => {
            if (res.state_info.status !== "OK") {
                _notifyService.error(res.state_info.message_body);
                return;
            }
    ...
    $("#btnDownloadMauBaoGia").click(function () {
        ...
        _motoCompensationService.downloadMauBaoGia(objBaoGiaChiTiet).then(res => {
    ...
    $("#file_upload_bao_gia").change(function () {
        ...
        if (formFields['file_upload_bao_gia'].files.length > 0) {
            ...
            _motoCompensationService.uploadBaoGia(formData).then(res => {
    ...
    $("#btnLuuDocOCRBaoGiaGara").click(function () {
        ...
        if (dem == 0) {
            _motoCompensationService.luuThongTinOCRBaoGiaGara(obj).then(res => {
                if (res.state_info.status !== "OK") {
                    _notifyService.error(res.state_info.message_body);
                    return;
                }
    ...
    $("#btnGaraBaoGiaSoSanh").click(function () {
        ...
        _motoCompensationService.laySoSanhBGGara(obj).then(res => {
            if (res.state_info.status !== "OK") {
                _notifyService.error(res.state_info.message_body);
                return;
            }
    ...
    $("#btnLapPASC").click(function () {
        ...
        _modalLapPhuongAnSuaChuaXeMayService.show(obj);
    });
...
$(document).bind('fscreenchange', function (e, state, elem) {
    if ($.fullscreen.isFullScreen()) {
        ...
        _modalThemHMTT.hide();
        _modalMotoClaimCompareData.hide();
    }
});
