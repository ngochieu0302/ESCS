function CarClaimCommon() {
    ...
    this.luuBoSungHoSoGiayTo = function (so_id, pm, callback = undefined) {
        ...
        if (_instance._frmAddHoSoGiayTo.isValid()) {
            _instance._carClaimCommonService.luuBsHoSoGiayTo(obj).then(res => {
    ...
    this.layDanhSachLoaiHsGiayTo = function (obj, callback = undefined) {
        var _instance = this;
        _instance._carClaimCommonService.layHoSoGiayTo(obj).then(res => {
            if (res.state_info.status !== "OK") {
                _instance._notifyService.error(res.state_info.message_body);
                return;
            }
    ...
    this.layDSThongTinTienTrinh = function (obj, callback = undefined) {
        var _instance = this;
        _instance._carClaimCommonService.layThongTinTienTrinh(obj).then(res => {
            if (res.state_info.status !== "OK") {
                _instance._notifyService.error(res.state_info.message_body);
                return;
            }
    ...
    this.showUocTonThat = function (data, el) {
        ...
        _service.postData("/carclaim/carclaimcommon/layUocTonThat", obj).then(res => {
            if (res.state_info.status !== "OK") {
    ...
    this.luuUocTonThat = function (callback = undefined) {
        ...
        if (_instance._frmUocTonThat.isValid()) {
            ...
            _service.postData("/carclaim/carinvestigation/luuUocTonThatNV", obj).then(res => {
                if (res.state_info.status !== "OK") {
                    _notifyService.error(res.state_info.message_body);
                    return;
                }
