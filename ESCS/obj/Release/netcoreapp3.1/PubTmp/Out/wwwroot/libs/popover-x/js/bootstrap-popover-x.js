...
}(function ($)
{
    ...
    PopoverButton.prototype = {
        ...
        listen: function (event)
        {
            ...
            if (isHover) {
                evIn = event === 'hover' ? 'mouseenter' : 'focusin';
                evOut = event === 'hover' ? 'mouseleave' : 'focusout';
                ...
                $h.handler($el, event, function (e) {
                    if (event === 'keyup') {
                        if ($dialog && e.which === 27) {
                            $h.raise($dialog, event).popoverX('hide');
                        }
                        return;
                    }
                    if (href && event === 'click') {
                    ...
                    $h.raise($dialog, event).popoverX('toggle');
    ...
    PopoverX.prototype = $.extend({}, $.fn.modal.Constructor.prototype, {
        ...
        getPlacement: function ()
        {
            ...
            var scrollTop = Math.max(db.scrollTop || 0, de.scrollTop);
            var isH = placement === 'horizontal';
            var scrollLeft = Math.max(db.scrollLeft || 0, de.scrollLeft);
            var isV = placement === 'vertical';
            var pageX = Math.max(0, pos.left - scrollLeft);
            var pageY = Math.max(0, pos.top - scrollTop);
            var autoPlace = placement === 'auto' || isH || isV;
            var width = window.innerWidth || de || document.body.clientWidth;
        ...
        refreshPosition: function () {
            ...
            var placement = self.getPlacement();
            switch (placement) {
                case 'bottom':
        ...
        show: function (skipValidation) {
            ...
            if (self.options.fullscreen && self.options.fullscreen !== '') {
                if (!$.fullscreen.isFullScreen()) {
                    $dialog.css(self.options.dialogCss).appendTo(self.$body);
                } else {
                    $dialog.css(self.options.dialogCss).appendTo($(self.options.fullscreen));
                }
            }
            if (!skipValidation) {
    ...
    $.fn.popoverButton = function (option) {
        ...
        return self.each(function ()
        {
            ...
            var options = $.extend({}, $.fn.popoverButton.defaults, $this.data(), typeof option === 'object' && option);
            if (!data) {
